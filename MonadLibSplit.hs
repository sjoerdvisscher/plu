...
module MonadLibSplit (
  ...
  -- ** Nested Execution
  -- $Nested_Exec
  RunReaderM(..), RunWriterM(..), RunExceptionM(..), RunMonadPlus(..),
...
class MonadPlus m => RunMonadPlus m where
  msplit :: m a -> m (Maybe (a, m a))

instance (Monad m) => RunMonadPlus (ChoiceT m) where
  msplit m = lift (runChoiceT m)

instance (RunMonadPlus m) => RunMonadPlus (IdT m) where
  msplit (IT m) = IT ((liftM . fmap . fmap) IT $ msplit m)
instance (RunMonadPlus m) => RunMonadPlus (ReaderT i m) where
  msplit (R m) = R ((liftM . fmap . fmap) (R . const) . msplit . m)
instance (RunMonadPlus m, Monoid i) => RunMonadPlus (WriterT i m) where
  msplit (W m) = W (liftM swap (msplit m))
    where swap Nothing           = P Nothing mempty
          swap (Just (P a w, m)) = P (Just (a, W m)) w
instance (RunMonadPlus m) => RunMonadPlus (StateT i m) where
  msplit (S m) = S (\s -> liftM (swap s) (msplit (m s)))
    where swap s Nothing          = (Nothing, s)
          swap _ (Just (~(a,s), m)) = (Just (a, S (const m)), s)
instance (RunMonadPlus m) => RunMonadPlus (ExceptionT i m) where
  msplit (X m) = X (liftM swap (msplit m))
    where swap Nothing             = Right Nothing
          swap (Just (Right a, m)) = Right $ Just (a, X m)
          swap (Just (Left e,  _)) = Left e
